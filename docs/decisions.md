...
## DEC-0001 — Keep config-repo feature files local
- Date: 2026-06-15
- Status: accepted
- Context: This repo is both an OpenCode configuration and a project template. Auto may create local feature files while improving the config itself, but those feature records are not part of the distributed template contract.
- Decision: Root `features/` in this config repo stays gitignored. Project repos initialized from `project/` may track `features/*.md` so feature state can be shared with that project's team.
- Consequences: Keep root `.gitignore` scoped to this repo's `features/`; keep `/init` guidance that unignores Markdown feature files in downstream project repos.
...
- Consequences: Safety must come from the protocol: rollback before destructive/outward actions, scoped feature files, verification, review, and disclosure. Do not silently harden permissions without a new product decision.
...
- Decision: Run `adversarial-reviewer` at the verification boundary for completed durable work on the default non-urgent path: feature Phase 4 Verify and no-feature durable-change closeout. Keep it available earlier on demand, but do not require it after every Build slice.
...
- Related: `agents/auto.md`, `templates/feature-template.md`, `README.md`, `project/SAMPLE-FEATURE.md`

## DEC-0005 — Make Auto an orchestration-first primary agent
- Date: 2026-06-16
- Status: accepted
- Context: Auto previously did most work itself while delegating specific searches, patches, and review gates. The intended operating model is now for Auto to keep the big-picture feature state while subagents perform more bounded research, implementation, impact mapping, test strategy, and review work.
...
- Consequences: Read-only lanes may run in parallel. Write lanes remain serialized unless paths/contracts are provably disjoint and the feature records why parallel writes are safe. Subagent output remains advisory until Auto verifies scope, evidence, and gates.
- Related: `agents/auto.md`, `agents/impact-mapper.md`, `agents/test-strategist.md`, `templates/feature-template.md`, `README.md`
...
- Decision: Add a `### Critical Thinking` section to `agents/auto.md` (placed before Step 0) defining four concrete moves — question the premise, surface uncertainty and trace claims, steel-man the opposite, pre-mortem non-trivial actions — woven into the existing 8-step cycle with no new sequential gates. Reframe Auto's initiative as thinking-primary, action-as-follow-through (opening paragraph + one clause in `description:`). Add `AGENTS.md §1.13` as a policy pointer. Add a Critical thinking row to the README behavior table and sharpen the Auto description line. Add an `Assumptions:` line to the feature template `## Design`. Align the `project/AGENTS.md` template.
...
- Related: `agents/auto.md`, `AGENTS.md`, `README.md`, `project/AGENTS.md`, `features/simplicity-lens.md`
