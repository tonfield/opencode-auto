...
- Related: `project/AGENTS.md`, `project/docs/README.md`

## Feature-file tracking differs between this repo and generated projects
- Symptom: Guidance can look contradictory because root `.gitignore` ignores `features/`, while `/init` recommends unignoring `features/*.md`.
- Why it happens: This config repo keeps its own Auto work records local, but downstream project repos often need feature files committed as shared project state.
- What to check: Root `.gitignore`, `commands/init.md`, README project-template docs, and any feature-file policy wording.
- Safe practice: Say “this config repo's `features/` is local” when describing root behavior; say “project repos may track `features/*.md`” when describing `/init` output.
- Related: `.gitignore`, `commands/init.md`, `README.md`, `docs/decisions.md#dec-0001--keep-config-repo-feature-files-local`
...
- Why it happens: `opencode-snip` publishes a stricter engine range than this local Node 22 environment, and npm's script-approval feature tracks packages with install scripts.
...
- Symptom: A decision record or feature file that stakes its credibility on "honestly documenting" a limitation gets the count wrong — overstating what's enforced and understating what's model-dependent.
...
- Related: `docs/decisions.md` (DEC-0006), `features/critical-thinking-system.md`

## grep honors .gitignore — feature-file sync claims must scope the path
- Symptom: A `[verified]` claim that "zero stale references remain" across all files is false when the file being verified lives under a gitignored path (e.g., `features/`).
- Why it happens: The `grep` tool (ripgrep-based) honors `.gitignore` by default. In this repo, root `.gitignore` ignores `features/`, so any repo-root grep silently skips every feature file — including the one the verification claim lives in.
...
- Safe practice: Never scope a sync-verification grep to the repo root alone when the surface includes `features/`. Always add a separate `rg --no-ignore features/` pass, or read the feature file directly.
- Related: `.gitignore`, `features/`, `docs/decisions.md#dec-0006`
...
- Safe practice: Write the sync gate as an explicit phrase-list, not a single-label check. Treat guardrail wording as load-bearing — it's the defense against the feature's own stated failure mode, so drift there is higher-stakes than naming drift.
- Related: `docs/decisions.md#dec-0007`, `features/simplicity-lens.md`

## Sync-grep scoping has dimensions beyond gitignore — path subtree and case
- Symptom: A "zero stale references" verification claim is false because the grep missed a restatement in a load-bearing path subtree (e.g. `project/` template files, `agents/`), or missed a capitalized variant (e.g. "Append-only" against a lowercase `append-only` pattern).
- Why it happens: ripgrep is case-sensitive by default. The verifier mentally scopes to "the files I just edited" and forgets sibling surfaces that restate the policy — especially the `project/` template subtree, which is load-bearing because README tells users to `cp -r project/* .` into new projects, so stale template policy is inherited verbatim downstream. Sentence-initial capitalization then hides the match from a lowercase pattern.
- What to check: For any "zero stale references" claim, run the grep case-insensitively (`rg -i`), with `--no-ignore` if `features/` is in scope, and explicitly enumerate every load-bearing subtree (`agents/`, `project/`, `templates/`, `commands/`, `docs/`), not just the canonical file you edited.
...
- Related: `project/docs/decisions.md`, `.gitignore`, `docs/decisions.md`, `docs/gotchas.md` (#5 grep-honors-gitignore, #6 surface-sync-phrase-scope)
