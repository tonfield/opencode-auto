...
**Goal:** Avoid shell-level prompts and hangs while preserving user-facing approval for destructive, credentialed, or ambiguous actions.
...
1. **Process Continuity (Turn-Taking):**
   ...
   - **Mechanism:** Prefer documented non-interactive flags, explicit arguments, and timeouts. Use chat/question for real decisions.
...
2. **Explicit Action Framing (Positive Constraints):**
   ...
   - **Mechanism:** Supply non-interactive flags that preserve safety; do not use force/destructive flags merely to avoid asking.
...
3. **No Shell Prompts**: Use documented non-interactive flags; avoid `--force`, `-f`, or `yes |` unless the action is already known safe and bounded.
...
| **Add** | `git add -p` | `git add <file>` for intended files |
...
| Tool | Interactive (BAD) | Non-Interactive (GOOD) |
|------|-------------------|------------------------|
| **RM** | `rm -i file` | Prefer OpenCode file tools; otherwise remove only explicitly verified paths without `-f` unless approved |
| **CP** | `cp -i a b` | Prefer OpenCode file tools; otherwise copy to a verified non-conflicting path |
| **MV** | `mv -i a b` | Prefer OpenCode file tools; otherwise move to a verified non-conflicting path |
| **Unzip** | `unzip file.zip` | Unzip into a fresh temp/output directory, or use overwrite flags only when the destination is disposable |
| **Tar** | `tar xf file.tar` | `tar xf file.tar` (usually safe) |
| **SSH** | `ssh host` | `ssh -o BatchMode=yes host`; do not disable host-key checks unless explicitly approved |
...
When a command doesn't have a non-interactive flag, prefer a different command, an OpenCode tool, or a user-facing question. Use the patterns below only when inputs are deterministic and safe.

### The "Yes" Pipe (rare, safe prompts only)
```bash
...
Never pipe passwords or secrets into shell commands. If credentials or elevated privileges are required, stop and ask the user for the appropriate approved path.

## 6. Best Practices

1. **Always test commands** mentally for interactive prompts before running
2. **Check docs/help** for safe non-interactive flags such as `--non-interactive`, `--no-input`, `--ci`, or explicit message/path arguments
...
2. **Follow the Specifics:** Use this file to refine shell behavior while still obeying higher-priority system, developer, user, and tool instructions.
